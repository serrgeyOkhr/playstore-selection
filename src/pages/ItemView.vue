        ...
        <div class="container" >
          <div class="itemTitle flex">
            <div class="gameImgs q-pr-lg">
              <div>
                <div class="q-mb-md">
                  <q-img fit :src="steamInfo?.header_image"/>
                </div>
                <div>
                  <q-carousel
                    swipeable
                    ...
                    navigation
                  >
                    <q-carousel-slide v-for="(img, index) in steamInfo?.screenshots" :key="index" :name="index"  :img-src="img.path_thumbnail" />
                  </q-carousel>
                </div>
              </div>
            ...
            <div class="gameInfo flex">
              ...
              <div class="price_place q-mt-md">
                ...
                <div class="card_container">
                  <q-card class="my-card q-ma-md" v-for="(game,index) in gameInfo?.deals" :key="index">
                      <!-- <q-img
                      fit="contain"
                      height="150px"
                      :src="game.thumb"
                    ...
                    <q-card-section>
                      ...
                      <div class="descriptionContainer text-caption q-mb-md">
                        <div class="storeImgContainer">
                          <q-img
                            :src="'https://www.cheapshark.com/'+stores.filter((shop) => game.storeID === shop.storeID )[0]?.images.logo"
                            alt=""
                          />
                        </div> 
                        <div>
                          <span v-if="game.retailPrice - game.price != 0" class="text-red-5"> Распродажа! </span><br>
                          <span class="text-body1">В магазине {{stores.filter((shop) => game.storeID === shop.storeID)[0]?.storeName}}</span>
                        </div>
                      </div>
                      <div class="price">
                        Цена:
                        <span v-if="game.retailPrice - game.price != 0" class="normalPrice q-mr-sm text-strike">{{game.retailPrice}}$</span>
                        <span class="salePrice text-h5 text-red-7">{{game.price}}$</span>
                      </div>
                    </q-card-section>
